...
/**
 * Hook untuk mengelola form perbarui jadwal (UPDATE).
 * Termasuk: populate form dari jadwalData, validasi operasional, dan submit.
 *
 * @param {Object|null} jadwalData        - Data jadwal yang sedang diedit
 * @param {Array}       existingSchedules - Jadwal lain (untuk blokir overlap, kecuali jadwal saat ini)
...
export function useEditJadwal(jadwalData, existingSchedules = [], onSuccess, isOpen) {
  ...
  // Populate form saat jadwalData berubah (tombol Edit ditekan)
  useEffect(() => {
    if (jadwalData) {
  ...
  // Fetch data klinik setiap kali modal dibuka
  useEffect(() => {
    if (isOpen) {
      fetchClinicData();
    }
  ...
  // ─── Disable Logik TimePicker (skip jadwal yang sedang diedit) ─
  const shouldDisableJamMulai = (timeValue, clockType) => {
    ...
    for (const s of existingSchedules) {
      // Lewati jadwal yang sedang diedit
