...
/**
 * PLANG PERINGATAN PEMUSNAHAN ULASAN (ModalHapus)
 * Ibarat plang peringatan bersimbol seru (!) besar yang dipasang mandor di depan mading.
 * Plang ini menanyakan ketegasan pimpinan: "Benarkah Anda ingin merobek dan memusnahkan ulasan ini?".
 * Jika menekan "Ya, Hapus", Asisten Pengawal Plang (useModalHapus) langsung memerintahkan petugas merobeknya.
 */
const ModalHapus = ({ isOpen, onClose, data, refetch, showToast }) => {
  // Meminta tombol eksekusi dan rambu sibuk dari Asisten Pengawal Plang
  const { isDeleting, handleDelete } = useModalHapus(data, refetch, showToast, onClose);

  if (!isOpen) return null; // Jika saklar ditutup, plang peringatan ini disembunyikan
