...
/**
 * TANDA PERINGATAN PENCABUTAN KEGIATAN MADING (ModalHapusKegiatan)
 * Ibarat plang peringatan bersimbol seru (!) besar yang dipasang mandor di depan papan mading.
 * Plang ini menanyakan ketegasan admin: "Benarkah Anda ingin menurunkan pengumuman ini permanen?".
 * Jika disetujui, Petugas Pencabut (handleDelete) langsung melepas paku madingnya.
 */
const ModalHapusKegiatan = ({ isOpen, onClose, hook }) => {
  const { isLoading, handleDelete } = hook;
  if (!isOpen) return null; // Jika saklar ditutup, plang peringatan disembunyikan

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white w-full max-w-md rounded-2xl p-8 text-center shadow-xl">
        
        {/* Simbol seru besar tanda bahaya */}
        ...
        {/* Pilihan ketegasan: Hapus atau Batalkan */}
