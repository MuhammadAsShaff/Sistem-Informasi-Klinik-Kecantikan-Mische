...
export default function DetailDokterPage() {
  const { id } = useParams();
  const { getDoctorById } = useDokterData();
  ...
  // Generate email dummy berdasarkan nama depan
  const firstName = doctor.name.replace('Dr. ', '').split(' ')[0];
  const email = `${firstName.charAt(0) + firstName.slice(1).toLowerCase()}@Gmail.Com`;

  // Format nama lengkap untuk diletakkan di dalam paragraf (Title Case)
  const fullNameTitleCase = doctor.name
    .replace('Dr. ', '')
    .split(' ')
