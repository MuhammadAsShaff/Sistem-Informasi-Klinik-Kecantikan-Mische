...
/**
 * Galeri kegiatan klinik dengan carousel — pure UI.
 * Semua data & navigasi dari hook `useGallerySection`.
 */
    ...
    <div className="w-full bg-gradient-to-r from-[#56bc36] from-[30%] to-[#C6FFD1] relative overflow-hidden py-12 md:py-24">
      ...
      <div className="container mx-auto px-6 md:px-10 flex flex-col items-center relative z-10 text-white">
        <div className="w-full text-left md:text-center mb-8 md:mb-12">
          ...
          <p className="text-sm md:text-base opacity-90 tracking-wide uppercase font-semibold">
          ...
          {kegiatanList.length > 1 && (
            <>
              <button onClick={handlePrev}
                className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 p-1.5 md:p-3 bg-white/30 backdrop-blur-md rounded-full text-white hover:bg-white/50 transition opacity-0 group-hover:opacity-100">
                <ChevronLeft size={28} />
              </button>
              <button onClick={handleNext}
                className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 p-1.5 md:p-3 bg-white/30 backdrop-blur-md rounded-full text-white hover:bg-white/50 transition opacity-0 group-hover:opacity-100">
