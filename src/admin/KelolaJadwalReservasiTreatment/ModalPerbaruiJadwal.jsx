...
export default function ModalPerbaruiJadwal({ isOpen, onClose, jadwalData, onSuccess, existingSchedules = [] }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const token = localStorage.getItem('token');
      const res = await axios.put(`http://127.0.0.1:8000/api/admin/schedules/${jadwalData.idJadwal || jadwalData.id}`, 
      {
        jamMulai,
        jamSelesai
      }, 
